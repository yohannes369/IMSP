...
//         // Group records by RequestID and Staff
//         const grouped = {};
//         rawRecords.forEach(r => {
//           const key = `${r.RequestID}-${r.StaffID}`;
//           if (!grouped[key]) grouped[key] = { staff: r, items: [] };
...
//       alert(`Item with FormID ${formID} returned successfully`);
//       setRecords(prev =>
//         prev.map(group => ({
//           ...group,
...
//   if (!records.length) return <p className="p-4">No records found.</p>;

//   return (
//     <div className="p-6 max-w-6xl mx-auto bg-white shadow rounded">
//       <h2 className="text-xl font-bold mb-4">Issued Items (Model22Form)</h2>
//       <table className="w-full table-auto border-collapse border border-gray-300">
//         <thead>
//           <tr className="bg-gray-200">
//             <th className="border p-2">RequestID</th>
...
//             <th className="border p-2">Action Date</th>
...
//                 {index === 0 && (
//                   <>
//                     <td className="border p-2" rowSpan={items.length}>{staff.RequestID}</td>
//                     <td className="border p-2" rowSpan={items.length}>
//                       {staff.FName} {staff.LName} ({staff.StaffID})
//                     </td>
//                   </>
//                 )}
//                 <td className="border p-2">{item.ItemName} ({item.ItemID})</td>
//                 <td className="border p-2">{item.UnitID}</td>
//                 <td className="border p-2">{item.SerialNo}</td>
//                 <td className="border p-2">{item.Quantity}</td>
//                 <td className="border p-2">{item.UnitPriceBirr}.{item.UnitPriceCent} Birr</td>
//                 <td className="border p-2">{item.BalanceQty}</td>
//                 <td className="border p-2">{item.Remark}</td>
//                 <td className="border p-2">{item.ActionType}</td>
//                 <td className="border p-2">{new Date(item.ActionDate).toLocaleString()}</td>
