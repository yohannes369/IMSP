...
//     { path: '/mdashboard', name: 'Dashboard', icon: <FaTh className="mr-2" /> },
//     { path: '/manager/request', name: 'Requests', icon: <FaClipboardList className="mr-2" />, hasNotification: true },
//     { path: '/manager/contactmessage', name: 'contactfromguestt', icon: <FaBoxes className="mr-2" /> },
...
//     { path: '/mdashboard', name: 'Dashboard', icon: <FaTh className="mr-2" /> },
//     { path: '/manager/request', name: 'Requests', icon: <FaClipboardList className="mr-2" />, hasNotification: true },
//     { path: '/manager/contactmessage', name: 'Contact from Guests', icon: <FaBoxes className="mr-2" /> },
//     { path: '/manager/inventoryreports', name: 'Inventory Reports', icon: <FaChartBar className="mr-2" /> },
//     { path: '/manager/return', name: 'return status', icon: <FaUsers className="mr-2" /> },
//   ];
...
const ManagerSidebar = () => {
  ...
  // Polling: fetch pending requests
  ...
  useEffect(() => {
    const handleNewRequest = () => setPendingRequestsCount(prev => prev + 1);
    const handleRequestResolved = () => setPendingRequestsCount(prev => (prev > 0 ? prev - 1 : 0));

    const handleNewReturn = () => setPendingReturnsCount(prev => prev + 1);
    const handleReturnResolved = () => setPendingReturnsCount(prev => (prev > 0 ? prev - 1 : 0));

    socket.on('new_request', handleNewRequest);
    socket.on('request_resolved', handleRequestResolved);
    socket.on('new_return', handleNewReturn);
    socket.on('return_resolved', handleReturnResolved);

    return () => {
      socket.off('new_request', handleNewRequest);
      socket.off('request_resolved', handleRequestResolved);
      socket.off('new_return', handleNewReturn);
      socket.off('return_resolved', handleReturnResolved);
    };
  }, []);

  const menuItems = [
    { path: '/manager', name: 'Dashboard', icon: <FaTh className="mr-2" /> },
    { path: '/manager/request', name: 'Requests', icon: <FaClipboardList className="mr-2" />, hasNotification: true, notificationCount: pendingRequestsCount },
    { path: '/manager/contactmessage', name: 'Contact from Guests', icon: <FaBoxes className="mr-2" /> },
    { path: '/manager/inventoryreports', name: 'Inventory Reports', icon: <FaChartBar className="mr-2" /> },
    { path: '/manager/return', name: 'Return Status', icon: <FaUsers className="mr-2" />, hasNotification: true, notificationCount: pendingReturnsCount },
  ];
